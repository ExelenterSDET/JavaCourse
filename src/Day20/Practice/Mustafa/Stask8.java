...
public class Stask8 {
 ...
 /*Task8:
        ...
        String [] fee=str.split(" ",0);

        System.out.println("fee = " + Arrays.toString(fee));

        double dollar = 0.0,pound = 0.0,euro = 0.0,lira = 0.0,empty = 0.0;

        for (String itemPrice : fee) {
            if (itemPrice.contains("$")) {
                ...
                pound += (1.8) * Double.parseDouble(itemPrice.replaceAll("[^0-9.]", ""));
        ...
        double Total = dollar + euro + pound + lira + empty;
