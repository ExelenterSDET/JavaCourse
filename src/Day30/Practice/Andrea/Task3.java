...
public class Task3 {
    public static void main(String[] args) {
        ...
        System.out.println(listToCamelCase(listOfLists));
}
    static ArrayList<ArrayList<String>> listToCamelCase (ArrayList<ArrayList<String>> listOfList){
        for (int row = 0; row < listOfList.size(); row++) {
            for (int col = 0; col < listOfList.get(row).size(); col++) {
                String s = listOfList.get(row).get(col);
                listOfList.get(row).set(col, s.substring(0,1).toUpperCase()+s.substring(1).toLowerCase());
            }
        }
